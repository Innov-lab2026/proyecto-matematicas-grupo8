...
let db = {
  ...
  opcion: [
    {
      id: 1,
      texto: "Respuesta de prueba (Incorrecta)",
      puntos: 0,
      escenarioId: 1,
    },
...
const mockPrisma = {
  usuario: {
    findUnique: async ({ where }) =>
      db.usuario.find((u) => u.id === where.id || u.email === where.email),
    upsert: async ({ where, create, update }) => {
      let idx = db.usuario.findIndex(
    ...
    update: async ({ where, data }) => {
      ...
      if (data.puntos?.increment)
        db.usuario[idx].puntos += data.puntos.increment;
      if (data.tokens?.increment)
        db.usuario[idx].tokens += data.tokens.increment;
  ...
  seccion: {
    findMany: async ({ where, include } = {}) => {
      return db.seccion.map((s) => ({
        ...s,
        escenarios: include?.escenarios
          ? db.escenario.filter((e) => e.seccionId === s.id)
          : [],
      }));
    },
    findUnique: async ({ where }) => {
  ...
  escenario: {
    findMany: async ({ where }) =>
      db.escenario
        ...
        .map((e) => ({
          ...e,
          opciones: db.opcion.filter((o) => o.escenarioId === e.id),
        })),

    findFirst: async ({ where }) => {
      const esc = db.escenario.find(
        (e) => e.id === where.id && e.seccionId === where.seccionId,
      );
      if (esc) esc.opciones = db.opcion.filter((o) => o.escenarioId === esc.id);
      return esc;
    },
    findUnique: async ({ where }) =>
      db.escenario.find((e) => e.id === where.id),
    create: async ({ data }) => {
  ...
  opcion: {
    findUnique: async ({ where }) => {
      ...
      if (op) {
        ...
        op.escenario.seccion = db.seccion.find(
          (s) => s.id === op.escenario.seccionId,
        );
      }
      return op;
    },
  ...
  progreso: {
    findFirst: async ({ where }) =>
      db.progreso.find(
        (p) =>
          p.usuarioId === where.usuarioId &&
          p.escenarioId === where.escenarioId,
      ),
    create: async ({ data }) => {
      const newP = {
        id: db.progreso.length + 1,
        ...data,
        updatedAt: new Date(),
      };
    ...
    update: async ({ where, data }) => {
      ...
      if (data.intentosFallidos?.increment)
        db.progreso[idx].intentosFallidos += data.intentosFallidos.increment;
      Object.keys(data).forEach((k) => {
    ...
    findMany: async ({ where }) =>
      db.progreso.filter((p) => p.usuarioId === where.usuarioId),
  },
  seccionAprobada: {
    findUnique: async ({ where }) =>
      db.seccionAprobada.find(
        (sa) =>
          sa.usuarioId === where.usuarioId_seccionId.usuarioId &&
    ...
    findMany: async ({ include } = {}) => {
      return db.auditoria.map((log) => ({
        ...
        usuario: include?.usuario
          ? db.usuario.find((u) => u.id === log.usuarioId)
          : null,
      }));
    },
    create: async ({ data }) => {
      ...
      if (!fileExists || isEmpty) {
        fs.writeFileSync(
          AUDIT_PATH,
          "id,usuarioId,accion,entidad,entidadId,detalles,timestamp",
        );
      }
